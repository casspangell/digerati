...
color adjustColorWithAmplitude(float amplitude, float time) {
  ...
  return color(hueValue, 100, brightness);
}
...
void draw() {
  ...
  for (int y = 0; y < rows - 1; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      ...
      float elapsedTime = (millis() - fadeStart)/10000.0;
      int colorChange = int(elapsedTime*100)%255;
      int colorChangeTwo = int(elapsedTime);
      println("elapsedTime "+elapsedTime);
      println("colorChange "+colorChange);
      ...
      float alpha = map(y, 0, rows - 1, 0, colorChange);
      vertexColor = color(red(colorChange), green(colorChangeTwo), blue(colorChange), alpha);
      stroke(vertexColor);
      fill(vertexColor);
