...
color adjustColorWithAmplitude(float amplitude, float time) {
  ...
  //float hueValue = map(amplitude*100, 1, 50, 50, 255); // Set hue value for orange range (30 to 50)
  //float saturation = map(amplitude*elapsedTime, 255, 255, 255, 255); // Adjust saturation for varying colors
  //float brightness = map(amplitude, 0, 1, 1, 255); // Adjust brightness for varying colors
  return color(colorTime%255, elapsedTime%255, 255);
}
...
void draw() {
  ...
  for (int y = 0; y < rows - 1; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      ...
      int elapsedTime = millis() - fadeStart;
